...
function listar_proyecto(){
     table = $("#tabla_proyecto").DataTable({
       "ordering":false,
       "paging": false,
...
$('#tabla_proyecto').on('click','.editar', function(){
 ...
 /* VER ACTIVIDAD */
 $('#tabla_proyecto').on('click','.veractividad', function(){
    var data=table.row($(this).parents('tr')).data();
    if(table.row(this).child.isShown()){
        var data=table.row(this).data();
    }
...
/* imprimir proyecto*/
$('#tabla_proyecto').on('click','.pdf', function(){
    var data=table.row($(this).parents('tr')).data();
    if(table.row(this).child.isShown()){
        var data=table.row(this).data();
    }
    window.open("../vista/MPDF/generar_proyecto.php?id="+parseInt(data.id_proyecto)+"#zoom=100%","Ticket","scrollbars=NO");
})
...
function listar_actividad(id){
    tableactividad = $("#tabla_ver_actividad").DataTable({
       ...
       "processing": true,
       "ajax":{
           "url":"../controlador/actividad/controlador_actividad_listar_proyecto.php",
           type:'POST',
           data:{
               id:id
           }
